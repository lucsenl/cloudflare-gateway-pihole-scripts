...
export const DELETION_ENABLED = !!parseInt(process.env.NOADS_DELETION_ENABLED, 10);
...
export const getTierListPrefix = (tier) =>
  `NOADS ${tier.charAt(0).toUpperCase() + tier.slice(1)}`;

export const getTierRuleName = (tier) =>
  `NOADS Filter ${tier.charAt(0).toUpperCase() + tier.slice(1)}`;
