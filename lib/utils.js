...
/**
 * Sends a NOADS notification to a Discord-compatible webhook.
...
export async function notifyWebhook(msg) {
  ...
  if (webhook_url && webhook_url.startsWith('http')) {
    ...
    try {
      await sendMessageToWebhook(webhook_url, `NOADS: ${msg}`);
    } catch (e) {
...
export const fetchRetry = async (...args) => {
  ...
  while (attempts < maxAttempts) {
    ...
    try {
      ...
      console.warn(`An error occurred while making a web request: "${errorMessage}", retrying. Attempt ${attempts} of ${maxAttempts}.\nTHIS IS NORMAL IN MOST CIRCUMSTANCES. Refrain from reporting this as a bug unless the script doesn't automatically recover after several attempts.`);

      if (attempts >= maxAttempts) {
        await notifyWebhook(`An HTTP error has occurred (${response?.status ?? "unknown status"}) while making a web request. Please check the logs for further details.`);
        throw error;
      }
