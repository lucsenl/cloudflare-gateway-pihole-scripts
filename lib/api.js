 ...
 * @returns {Promise<Object>}
 */
export const upsertZeroTrustRule = async (name, filters, conditions) => {
 ...
 */
export const upsertZeroTrustDNSRule = async (lists, listName, prefix = "NOADS List", locationIds = []) => {
  ...
  await upsertZeroTrustRule(listName, ["dns"], conditions);
};
 ...
 * @param {string[]} [locationIds] Optional location IDs to scope the rule.
 */
export const upsertZeroTrustSNIRule = async (lists, listName, prefix = "NOADS List", locationIds = []) => {
  const filteredLists = lists.filter(({ name }) => name.startsWith(prefix));
  ...
  await upsertZeroTrustRule(listName, ["l4"], conditions);
};
