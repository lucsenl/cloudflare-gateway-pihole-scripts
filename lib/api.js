...
export const getZeroTrustLists = async () => {
  ...
  while (true) {
    console.log(`Fetching Zero Trust lists page ${page}...`);
    const response = await requestGateway(`/lists?page=${page}&per_page=${perPage}`, {
      method: "GET",
    });
    ...
    // Parar se não houver info, ou se já retornou todos os itens
    if (!info) break;
    const fetched = info.page * info.per_page;
    if (fetched >= info.total_count || results.length < perPage) break;
    page++;
  }
 ...
 * Inspects existing lists starting with the given prefix.
 ...
 * @param {string} prefix The list name prefix for this tier.
 */
export const synchronizeZeroTrustLists = async (items, prefix = "NOADS List") => {
  const itemSet = new Set(items);

  console.log(`Checking existing lists with prefix "${prefix}"...`);
  const { result: lists } = await getZeroTrustLists();
  const noadsLists = lists?.filter(({ name }) => name.startsWith(prefix)) || [];
  console.log(`Found ${noadsLists.length} existing lists. Calculating diffs...`);

  const domainsByList = {};
  // Do this sequentially to avoid rate-limits
  for (const list of noadsLists) {
  ...
  for(const [listId, patch] of Object.entries(patches)) {
    ...
    console.log(`Updating list "${noadsLists.find(list => list.id === listId).name}"${appends ? `, ${appends} additions` : ''}${removals ? `, ${removals} removals` : ''}`);
  ...
  if (toAdd.length) {
    ...
    const nextListNumber = Math.max(0, ...noadsLists.map(list => parseInt(list.name.replace(`${prefix} - Chunk `, ''))).filter(x => Number.isInteger(x))) + 1;
    await createZeroTrustListsOneByOne(toAdd, nextListNumber, prefix);
  }
};

/**
 * Defragment Zero Trust lists.
 * Inspects existing lists starting with "NOADS List - Chunk <number>"
 ...
 * @param {string} [prefix] The list name prefix for this tier.
 * @returns {Promise<Object|null>} A object that include the now empty and non-empty lists, or null if no lists found.
 */
export const defragmentZeroTrustLists = async (prefix = "NOADS List") => {
  console.log(`Checking existing lists with prefix "${prefix}"...`);
  const { result: lists } = await getZeroTrustLists();
  const noadsLists = lists?.filter(({ name }) => name.startsWith(`${prefix} - Chunk `)) || [];

  if (noadsLists.length === 0) {
    console.log(`No chunk lists found with prefix "${prefix}".`);
    return null;
  }
  console.log(`Found ${noadsLists.length} existing lists. Downloading...`);

  // Sort the lists by the natural number order in the name
  noadsLists.sort((a, b) => {
    const aNum = parseInt(a.name.replace(`${prefix} - Chunk `, ""));
    const bNum = parseInt(b.name.replace(`${prefix} - Chunk `, ""));
    return aNum - bNum;
  });
  ...
  console.log(`Found ${allEntries.length} entries in ${noadsLists.length} lists`);
  ...
  const assignedEntries = allEntries.map((entry, index) => {
    ...
    const assignedListId = noadsLists[listIndex]?.id || null;
    // The list should always exist since we're only shuffling the entries
    if (!assignedListId) {
      throw new Error(`Unable to resolve list for entry ${index}, have only ${noadsLists.length} lists`);
    }
  ...
  const emptyLists = noadsLists.filter(list => !assignedLists.has(list.id));
  // Gather the non-empty lists, using the original list not just the chunked ones
  // This is important to capture any manually created lists starting with the prefix
 ...
 * @param {string} [prefix] The list name prefix for this tier.
 */
export const createZeroTrustListsOneByOne = async (items, startingListNumber = 1, prefix = "NOADS List") => {
 ...
 * @param {Object[]} conditions The AST conditions for the rule.
 * @returns {Promise<Object>}
 */
export const upsertZeroTrustRule = async (name, filters, conditions, existingRules = null) => {
  ...
  if (existingRule) {
    ...
    return updateZeroTrustRule(existingRule.id, name, filters, conditions);
  }
  if (DEBUG) console.log(`No existing rule named "${name}", creating...`);
  return createZeroTrustRule(name, filters, conditions);
}
 ...
 * @returns {Promise}
 */
export const createZeroTrustRule = async (name, filters, conditions) => {
  try {
    await requestGateway("/rules", {
      ...
      body: JSON.stringify({
        name,
        description:
          "Filter lists created by NOADS. Avoid editing this rule. Changing the name of this rule will break the script.",
        enabled: true,
        action: "block",
        rule_settings: { "block_page_enabled": BLOCK_PAGE_ENABLED, "block_reason": "Blocked by NOADS, check your filter lists if this was a mistake." },
        filters,
        conditions,
      }),
    });
 ...
 * @returns {Promise<Object>}
 */
export const updateZeroTrustRule = async (id, name, filters, conditions) => {
  try {
...
/**
 * Builds AST conditions for DNS domain matching, optionally scoped to locations.
 * Uses the structured expression format (conditions) instead of wirefilter strings.
 * @param {object[]} filteredLists Lists with id and name.
 * @param {string[]} locationIds Optional location IDs.
 * @returns {Object[]} conditions array for the Cloudflare API.
 */
const buildDNSConditions = (filteredLists, locationIds) => {
  const domainExpressions = filteredLists.map(({ id }) => {
    const domainCheck = {
      any: { in: { lhs: { splat: "dns.domains" }, rhs: `$${id}` } },
    };
    ...
    if (locationIds.length > 0) {
      return {
        and: [
          domainCheck,
          { in: { lhs: "dns.location", rhs: locationIds } },
        ],
      };
    }
  ...
  const expression =
    domainExpressions.length === 1
      ? domainExpressions[0]
      : { or: domainExpressions };

  return [{ type: "traffic", expression }];
};

/**
 * Creates or Updates Zero Trust DNS rule for a given array of lists.
 * Uses AST conditions format for proper list + location binding.
 ...
 * @param {string} [prefix] The list name prefix to filter by.
 * @param {string[]} [locationIds] Optional location IDs to scope the rule.
 */
export const upsertZeroTrustDNSRule = async (lists, listName, prefix = "NOADS List", locationIds = [], existingRules = null) => {
  const filteredLists = lists.filter(({ name }) => name.startsWith(prefix));

  console.log(`Found ${filteredLists.length} lists with prefix "${prefix}" (out of ${lists.length} total).`);

  if (filteredLists.length === 0) {
    console.log(`No lists found with prefix "${prefix}" - Skipping DNS rule "${listName}".`);
    return;
  }

  const conditions = buildDNSConditions(filteredLists, locationIds);

  if (DEBUG) console.log(`AST conditions: ${JSON.stringify(conditions, null, 2)}`);
  console.log(`Upserting DNS rule "${listName}" (${filteredLists.length} lists${locationIds.length ? `, ${locationIds.length} locations` : ''})...`);
  await upsertZeroTrustRule(listName, ["dns"], conditions, existingRules);
};

/**
 * Builds AST conditions for SNI domain matching, optionally scoped to locations.
 * @param {object[]} filteredLists Lists with id and name.
 * @param {string[]} locationIds Optional location IDs.
 * @returns {Object[]} conditions array for the Cloudflare API.
 */
const buildSNIConditions = (filteredLists, locationIds) => {
  const sniExpressions = filteredLists.map(({ id }) => {
    const sniCheck = {
      any: { in: { lhs: { splat: "net.sni.domains" }, rhs: `$${id}` } },
    };
 ...
 * @param {string[]} [locationIds] Optional location IDs to scope the rule.
 */
export const upsertZeroTrustSNIRule = async (lists, listName, prefix = "NOADS List", locationIds = [], existingRules = null) => {
  const filteredLists = lists.filter(({ name }) => name.startsWith(prefix));

  console.log(`Found ${filteredLists.length} lists with prefix "${prefix}" (out of ${lists.length} total).`);

  if (filteredLists.length === 0) {
    console.log(`No lists found with prefix "${prefix}" - Skipping SNI rule "${listName}".`);
    return;
  }

  const conditions = buildSNIConditions(filteredLists, locationIds);

  if (DEBUG) console.log(`AST conditions: ${JSON.stringify(conditions, null, 2)}`);
  console.log(`Upserting SNI rule "${listName}" (${filteredLists.length} lists${locationIds.length ? `, ${locationIds.length} locations` : ''})...`);
  await upsertZeroTrustRule(listName, ["l4"], conditions, existingRules);
};
