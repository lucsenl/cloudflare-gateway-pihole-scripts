...
export const upsertZeroTrustDNSRule = async (lists, listName, prefix = "CGPS List", locationIds = []) => {
  ...
  // Create a Wirefilter expression to match DNS queries against the filtered lists
  let wirefilterDNSExpression = filteredLists
    .map(({ id }) => `any(dns.domains[*] in \$${id})`)
    .join(" or ");

  // Add location filter for non-core tiers
  if (locationIds.length > 0) {
    const locationFilter = `dns.location in {${locationIds.map(id => `"${id}"`).join(" ")}}`;
    wirefilterDNSExpression = `(${wirefilterDNSExpression}) and ${locationFilter}`;
  }

  if (DEBUG) console.log(`Wirefilter expression: ${wirefilterDNSExpression}`);
  console.log(`Upserting DNS rule "${listName}"...`);
...
export const upsertZeroTrustSNIRule = async (lists, listName, prefix = "CGPS List", locationIds = []) => {
  ...
  // Create a Wirefilter expression to match SNI queries against the filtered lists
  let wirefilterSNIExpression = filteredLists
  ...
  if (locationIds.length > 0) {
    const locationFilter = `dns.location in {${locationIds.map(id => `"${id}"`).join(" ")}}`;
    wirefilterSNIExpression = `(${wirefilterSNIExpression}) and ${locationFilter}`;
  }

  if (DEBUG) console.log(`Wirefilter expression: ${wirefilterSNIExpression}`);
  console.log(`Upserting SNI rule "${listName}"...`);
