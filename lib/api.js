 ...
 */
export const synchronizeZeroTrustLists = async (items, prefix = "CGPS List") => {
  ...
  if (toAdd.length) {
    ...
    const nextListNumber = Math.max(0, ...cgpsLists.map(list => parseInt(list.name.replace(`${prefix} - Chunk `, ''))).filter(x => Number.isInteger(x))) + 1;
...
export const defragmentZeroTrustLists = async (prefix = "CGPS List") => {
  console.log(`Checking existing lists with prefix "${prefix}"...`);
  const { result: lists } = await getZeroTrustLists();
  const cgpsLists = lists?.filter(({ name }) => name.startsWith(`${prefix} - Chunk `)) || [];
 ...
 * @param {string} [prefix] The list name prefix for this tier.
 */
export const createZeroTrustListsOneByOne = async (items, startingListNumber = 1, prefix = "CGPS List") => {
 ...
 */
export const upsertZeroTrustDNSRule = async (lists, listName, prefix = "CGPS List", locationIds = []) => {
 ...
 * @param {string[]} [locationIds] Optional location IDs to scope the rule.
 */
export const upsertZeroTrustSNIRule = async (lists, listName, prefix = "CGPS List", locationIds = []) => {
  const filteredLists = lists.filter(({ name }) => name.startsWith(prefix));
